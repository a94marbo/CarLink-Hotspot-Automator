      ...
      <div className="p-5 border-b border-slate-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Scan Web Bluetooth */}
            <button
              id="btn-scan-web-bluetooth"
              onClick={onScanWebBluetooth}
              disabled={isScanning}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 transition-colors disabled:opacity-50"
              title="Pair nearby Bluetooth car infotainment"
            >
              <Bluetooth className={`w-3.5 h-3.5 ${isScanning ? 'animate-spin text-blue-600' : 'text-blue-600'}`} />
              {isScanning ? 'Scanning...' : 'Pair Live BT'}
            </button>

            {/* Query previously granted browser BT devices */}
            <button
              id="btn-fetch-paired-bt"
              ...
              title="Query previously paired Bluetooth devices in browser"
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500 mt-1 flex-wrap">
                    ...
                    {device.isRealNativeDevice && (
                      <span className="text-[10px] text-blue-600 font-semibold bg-blue-50 border border-blue-200 px-1.5 py-0.2 rounded">
                        Web Bluetooth
                      </span>
                    )}
